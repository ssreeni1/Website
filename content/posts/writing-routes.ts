export const writingRoutes = [
  { slug: "diffusion-is-all-you-need", title: "WHO CARES? (Diffusion is All You Need)" },
  { slug: "five-lines", title: "Five Lines to Infinity" },
  {
    slug: "permanence-is-the-rarest-asset-class",
    title: "Permanence Is the Rarest Asset Class",
  },
  { slug: "when-everything-goes-to-zero", title: "when everything goes to 0" },
  {
    slug: "hyperspeculation-genesis-ii",
    title: "HYPERSPECUL(A)T(I)ON (GENESIS II)",
  },
  {
    slug: "shigetas-dream",
    title: "Shigeta's Dream: The Gachafication of Everything",
  },
  {
    slug: "genesis-i",
    title: "GENESIS Pt. I: What The F*ck Happened to Crypto x AI?",
  },
  { slug: "the-hedonists-stone", title: "The Hedonist's Stone" },
  {
    slug: "speculation-is-dead",
    title: "Speculation is Dead, Long Live Speculation",
  },
  { slug: "building-trading", title: "Building <-> Trading" },
] as const;
